...
source "${GOK_ROOT}/lib/utils/verbosity.sh" 2>/dev/null || true
source "${GOK_ROOT}/lib/utils/validation.sh" 2>/dev/null || true
source "${GOK_ROOT}/lib/utils/summaries.sh" 2>/dev/null || true
...
baseInst() {
    ...
    show_component_summary "base" "kube-system"
    log_component_success "base" "Base platform services installed successfully"
    log_success "Base platform installation completed in ${duration}s"

    # Show base-specific next steps
