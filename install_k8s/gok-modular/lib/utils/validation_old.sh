...
validate_docker_installation() {
    ...
    log_step "2" "Checking containerd service"
    if systemctl is-active --quiet containerd; then
        ...
        if [[ -n "$containerd_version" ]]; then
            echo -e "${COLOR_CYAN}📦 Containerd version: $containerd_version${COLOR_RESET}"
        fi
    else
    ...
    log_step "4" "Testing container creation"
    log_substep "Running hello-world container test (timeout: ${timeout}s)"
    if timeout "$timeout" docker run --rm hello-world >/dev/null 2>&1; then
        log_success "Container creation test passed"
    else
        log_warning "Docker hello-world test failed - may need internet connectivity"
        # Don't fail validation for this as it might be network-related
    fi
    
    log_step "5" "Checking Docker configuration"
    log_substep "Validating cgroup driver configuration"
    local cgroup_driver=$(docker info 2>/dev/null | grep "Cgroup Driver" | cut -d: -f2 | tr -d ' ')
    if [[ "$cgroup_driver" == "systemd" ]]; then
        log_success "Cgroup driver correctly set to systemd (Kubernetes compatible)"
    elif [[ -n "$cgroup_driver" ]]; then
        log_warning "Docker cgroup driver is not set to systemd (current: $cgroup_driver)"
        echo -e "${COLOR_YELLOW}⚠️  Recommendation: Configure systemd cgroup driver for Kubernetes compatibility${COLOR_RESET}"
    else
        log_warning "Unable to determine cgroup driver"
    fi
    
    # Additional configuration checks
    log_substep "Checking storage driver"
    ...
    if [[ -n "$storage_driver" ]]; then
        ...
        if [[ "$storage_driver" == "overlay2" ]]; then
            echo -e "${COLOR_GREEN}✓ Recommended storage driver for production${COLOR_RESET}"
        fi
    else
        log_warning "Unable to determine storage driver"
    fi
